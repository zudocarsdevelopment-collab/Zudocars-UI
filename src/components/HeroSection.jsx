...
const ROAD_PATH =
  "path('M 1150 160 C 950 260, 820 300, 760 380 C 690 470, 560 500, 420 560 C 260 630, 120 660, -80 770')";
...
const CONVOY = [
  ...
  {
    src: "https://nar.media.audi.com/is/image/audinar/country/us/en/assets/pct/2026/1920x1920_D2_MY27---Q6-e-tron---Front---Profile---Parked---5204.jpg?preferwebp=true",
    baseWidth: 210,
    duration: 9,
    delay: 5.4,
  },
  {
    src: "https://images.unsplash.com/photo-1568844293986-8d0400bd4745?auto=format&fit=crop&w=1100&q=85",
    baseWidth: 230,
    duration: 9,
    delay: 7.2,
  },
...
export default function Hero() {
  ...
  function handleBookingSubmit(e) {
    ...
    const params = new URLSearchParams({
      ...
      dropoff_location_id: (isDifferentDropoff
        ? dropLocationId
        : pickupLocationId
      ).toString(),
      vehicle_type: "car",
          ...
          >
            <div className="inline-flex items-center gap-3 bg-white/[0.03] border border-white/10 backdrop-blur-xl rounded-full pl-2 pr-5 py-1.5 mb-8">
              ...
              <span className="text-gray-400 text-xs font-semibold tracking-wide flex items-center gap-1">
                ZUDO — CAR RENTALS & DRIVER SERVICES{" "}
            ...
            <div className="relative overflow-hidden rounded-[32px] p-[1px] bg-gradient-to-b from-white/10 via-white/[0.02] to-transparent shadow-2xl shadow-black/80">
              <div className="relative bg-[#0d121f]/90 backdrop-blur-3xl rounded-[31px] p-6 sm:p-7">
                ...
                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <label className="block">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setPickupLocationId(Number(e.target.value))
                          }
                      ...
                      {isDifferentDropoff && (
                        <label className="block">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setDropLocationId(Number(e.target.value))
                            }
                            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2.5 text-white text-xs font-medium focus:outline-none focus:border-blue-500/50 transition-colors cursor-pointer"
                          >
                            {LOCATIONS.map((loc) => (
                  ...
                  {/* Rental Dates - Unified Popover DateRangePicker */}
                  <DateRangePicker
                  ...
                  >
                    {({ openPicker, formatDate }) => (
                      <button
                        type="button"
                        onClick={openPicker}
                        className="w-full bg-white/[0.04] border border-white/[0.08] hover:border-blue-500/50 hover:bg-white/[0.06] rounded-xl px-4 py-3 text-left text-white text-xs font-medium focus:outline-none focus:border-blue-500/50 transition-all cursor-pointer shadow-inner block"
                        aria-label="Select pickup and drop-off dates"
                      >
                        <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1.5">
                          <CalendarDays className="w-3.5 h-3.5 text-blue-400" />
                          Rental dates
                        </span>
                        <span className="flex items-center justify-between text-xs font-semibold text-white">
                          <span className="text-sm font-semibold">{formatDate(pickupDate) || "Pickup Date"}</span>
                          <span className="text-gray-500 font-normal">→</span>
                          <span className="text-sm font-semibold">{formatDate(dropDate) || "Drop Off Date"}</span>
                        </span>
                      </button>
                    )}
                  </DateRangePicker>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">
                    <div className="self-end flex flex-col justify-end bg-white/[0.02] border border-white/[0.06] rounded-2xl p-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <select
                          ...
                          className="col-span-2 w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-2 py-2 text-white text-xs font-medium focus:outline-none cursor-pointer"
                        >
                          {TIME_SLOTS.map((t) => (
                            <option
                              key={t.value}
